...
def build_manifests():
    ...
    bvbrc = _safe_read(meta/"bvbrc_genomes.tsv", sep="\t"); bvbrc["source"] = "bvbrc"; bvbrc["data_type"] = "assembly"
    sra   = _safe_read(meta/"ncbi_sra.tsv", low_memory=False); sra["source"] = "ncbi_sra"; sra["data_type"] = "reads"
    path  = _safe_read(meta/"ncbi_pathogen.tsv", sep="\t"); path["source"] = "pathogen"; path["data_type"] = "assembly"
    ena   = _safe_read(meta/"ena.tsv", sep="\t"); ena["source"] = "ena"; ena["data_type"] = "reads"

    asm_parts = [df for df in [bvbrc, path] if not df.empty]
    reads_parts = [df for df in [sra, ena] if not df.empty]
    asm = pd.concat(asm_parts, ignore_index=True) if asm_parts else pd.DataFrame()
    reads = pd.concat(reads_parts, ignore_index=True) if reads_parts else pd.DataFrame()
